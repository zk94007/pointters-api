...
describe('User services', () => {
    describe('SUCCESS', () => {
        it('/user/followers GET -> should return 200', async() => {
            ...
            const { body: { docs } } = await agent.get('/user/followers')
    ...
    describe('FAIL', () => {
        it('/user/followers GET -> should return 404', async() => {
            ...
            const authorizationHeader = { Authorization: `Bearer ${token}` };
            const Cookie = { Cookie: cookie };
            const { body: { message } } = await agent.get('/user/followers')
                .set(authorizationHeader)
                .set(Cookie)
                .expect(404);
            assert(message === 'No follower found');
        });
        it('/user/followers GET -> should return 401', async() => {
