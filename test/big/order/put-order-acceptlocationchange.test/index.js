...
describe('User requests', () => {
    describe('SUCCESS', () => {
        it('/order/:idOrder/accept-location-change PUT sohuld create a request given', async() => {
            const orderCreated = await createOrder(order);
            await agent.put(`/order/${orderCreated._id}/accept-location-change`)
                .send(orderCreated)
    ...
    describe('FAIL', () => {
        it('/order/:idOrder/accept-location-change PUT sohuld create a request given', async() => {
            ...
            await agent.put(`/order/1234567890qwertyuiopasdf/accept-location-change`)
                .send(order)
                .set(authorizationHeader)
                .set(Cookie)
                .expect(404);
